"""Log one M24-H controlled S2 replication trial state CSV from ROS2.
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Log one M24-H controlled S2 replication trial.")
    parser.add_argument("--trial-id", required=True)
    parser.add_argument("--replication-group-id", required=True)
    parser.add_argument("--condition", required=True, choices=[CONDITION])
    ...
    if args.mock:
        ...
        print(f"M24-H mock controlled state log written: {output_path}")
        return 0

    try:
        rows = collect_ros2_rows(args, total_sec)
    except Exception as exc:
        print(f"ERROR: ROS2 controlled replication logging failed: {exc!r}")
        write_rows(output_path, [])
        return 1

    write_rows(output_path, rows)
    print(f"M24-H ROS2 controlled state log written: {output_path} ({len(rows)} samples)")
...
def build_mock_rows(args: argparse.Namespace, total_sec: float) -> list[dict[str, str]]:
    ...
    for index in range(sample_count):
        ...
        rows.append(build_row(args, t_rel, mock_pose(args, t_rel), "m24h_mock_fixture"))
    return rows
...
def collect_ros2_rows(args: argparse.Namespace, total_sec: float) -> list[dict[str, str]]:
    ...
    node = rclpy.create_node("m24h_controlled_s2_replication_logger")
    ...
    try:
        while time.time() - t0_wall < total_sec:
            rclpy.spin_once(node, timeout_sec=0.1)
            t_rel = time.time() - t0_wall
